...
function updateLeaderboard(topUsers) {
	$.getJSON('setups.json?nocache=' + (new Date()).getTime(), function (data) {
		var color_values = ['white', 'white', '#ff8ff6', '#f1c40f', '#f1c40f'];
		html_str = '';
		console.log(topUsers);
		for (var userId in topUsers) {
                user = topUsers[userId];
			//if (userid == user["userid"] && user["rush_access"] == 1 && user["supporter"] < 2) {
			//    document.getElementById("error_sup").style.display = "block";
			//}
			if (!data['global']['banned'].includes(user['userid']) && user['rush_score'] != 0) {
				if (user['supporter'] >= 2) {
					color = color_values[user['supporter']];
				} else {
					color = 'white';
				}
				...
				var htmlData = `<a href="profile?userid=${user['userid']}"><li class="miniLeaderboardItem">
          <img class="leaderboard icon" src="https://cdn.discordapp.com/avatars/${user['userid']}/${user['avatar']}.webp" height="25" width="25" onerror="this.onerror=null; this.src='https://discordapp.com/assets/322c936a8c8be1b803cd94861bdfa868.png'">
          <span class="miniLeaderboardName" style="color:${color}">${user['username']}</span>
          <span class="miniLeaderboardPoints">${user['rush_score']} Puzzles</span>
        </li></a>`;
				html_str += htmlData;
			}
		}
...
window.addEventListener('load', function () {
	...
	tokenType = fragment.get('token_type');
	 url = 'https://twowi.de/getRush.php';
	// url = '/getRush1.json';
	...
	function init(json) {
		...
		$.getJSON('setups.json?nocache=' + (new Date()).getTime(), function (mainData) {
			globData = mainData;
		...
		for (var testKey in controls) {
			if (testKey == 'reset') {
				if (key.which == controls['reset'][0] && countdown < 2950 && lives != 0) {
					...
					http.timeout = 2000;
					http.open('POST', 'completedRush.php', true);
	...
	function start(data, mainData) {
		...
		if ('error' in data) {
			...
			}if(window.location.href.includes("?remmy=true")){
			    countdown = 15000;
			...
			function loop() {
				if (lives == 0) {
					...
					var http = new XMLHttpRequest();
					http.timeout = 2000;
					http.open('POST', 'saveRush.php', true);
					http.setRequestHeader('Content-Type', 'application/json');
					sendData = {
						id: data[board_id]['id'],
						win: false,
