...
$.getJSON('setups.json?nocache=' + (new Date()).getTime(), function( data ) {
  ...
  for (var c = 0; c < canvi.length; c++) {
    ...
    console.log(data[board_index[i][0]][board_index[i][1]]);
  ...
  setInterval(function(){
    ...
    Array.prototype.forEach.call(els, function(item, i) {
      ...
      if ( ((filters["show_legacy"]&&item.dataset.season !=3) || item.dataset.season == data.global.season) && ((filters["show_cleared"] && item.dataset.completed == "true") || (item.dataset.completed == "false")) && filters[item.dataset.difficulty] && (item.getElementsByClassName("description")[0].innerHTML.toLowerCase().includes(searched) || (item.getElementsByClassName("title")[0].innerHTML.toLowerCase().includes(searched))) && (check(item.getElementsByClassName("title")[0].innerHTML.toLowerCase()) || check(item.getElementsByClassName("description")[0].innerHTML.toLowerCase()))){
